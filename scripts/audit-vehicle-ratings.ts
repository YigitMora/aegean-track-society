...
const eliteReviewCodes = new Set([
  ...
  "lamborghini_huracan_performante",
  "mclaren_600lt",
  "dodge_viper_acr_vx",
  "chevrolet_corvette_zr1_c7",
]);
...
const sprint4UAlfaRomeoRows = extractVehicleRows("sprint4UAlfaRomeoVehicleDefinitions");
const sprint4UAlfaRomeoVehicleCodes = new Set(
  sprint4UAlfaRomeoRows.map((row) => row.code),
);
sprint4UAlfaRomeoRows.forEach((row) => {
  row.notes.push("Sprint 4U Alfa Romeo expansion; conservative source trail applied.");
});
const sprint4UDailyPerformanceRows = extractVehicleRows(
  "sprint4UDailyPerformanceVehicleDefinitions",
);
const sprint4UDailyPerformanceVehicleCodes = new Set(
  sprint4UDailyPerformanceRows.map((row) => row.code),
);
sprint4UDailyPerformanceRows.forEach((row) => {
  row.notes.push("Sprint 4U daily/performance expansion; not tuned to force leaderboard placement.");
});
const sprint4UEliteRows = extractVehicleRows("sprint4UEliteVehicleDefinitions");
const sprint4UEliteVehicleCodes = new Set(sprint4UEliteRows.map((row) => row.code));
...
export function detectHierarchyIssues(rows: VehicleAuditRow[]): AuditIssue[] {
  ...
  requireOverallLead(byCode, "ferrari_488_pista", "ferrari_458_speciale", issues);
  requireOverallLead(
    byCode,
    "lamborghini_huracan_sto",
    "lamborghini_huracan_performante",
...
function renderAuditDocument(rows: VehicleAuditRow[], issues: AuditIssue[]) {
  ...
  const sprint4UAlfaRows = rows.filter((row) =>
    sprint4UAlfaRomeoVehicleCodes.has(row.code),
  );
  const sprint4UDailyRows = rows.filter((row) =>
  ...
  return [
    ...
    `| Stock Overall >= 90 | ${stock90Count} |`,
    `| Stock Overall >= 95 | ${stock95Count} |`,
    ...
    "",
    renderVehicleTable(sprint4UAlfaRows),
    "",
    "## Sprint 4U Daily/Performance Templates",
    "",
    renderVehicleTable(sprint4UDailyRows),
    "",
